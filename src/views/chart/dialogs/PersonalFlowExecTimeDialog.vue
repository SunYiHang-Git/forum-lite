...
import { nextTick, onMounted, reactive, type Ref, ref } from 'vue'
...
async function loadData() {
  ...
  for (let i = 0; i < itemList.length; i++) {
    ...
    const execTimeList = userMap.get(userName)!.map((execTime) => Math.ceil((execTime / 3600) * 100) / 100)
...
function getInitData(): { userMap: IUserMap; monthList: string[] } {
  ...
  while (!table.eof()) {
    ...
    if (flowInfo) {
      ...
      execList[index] = execList[index] + table.i('ExecTime')
    }
    table.next()
  }
...
<template>
  ...
  >
    ...
    <div v-else class="infinite-list" style="overflow: auto">
      <template v-for="item in showUserItemList" :key="item.title">
        <ChartItem class="infinite-list-item" :echarts-options="item.options" :title="item.title" />
      </template>
    </div>
    <!-- 弹窗按钮 -->
    <template #footer>
